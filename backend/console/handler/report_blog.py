...
<<<<<<< HEAD
async def api_report_blog_post(request: ApiGetSavedBlogsPostRequest) -> ApiDeleteBlogPost200Response:
...
<<<<<<< HEAD
            BlogReport.create(db, blog_id=int(request.blog_id), user_id=int(user_id), reason=reason)
            logger.info(f"Blog reported: blog_id={request.blog_id} user_id={user_id} reason={reason!r}")
        else:
            logger.info(f"Blog already reported: blog_id={request.blog_id} user_id={user_id}")

        return ApiDeleteBlogPost200Response(blogID=request.blog_id)
    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"Failed to report blog: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail="Failed to report blog")
    finally:
        try:
            next(_db_gen)
