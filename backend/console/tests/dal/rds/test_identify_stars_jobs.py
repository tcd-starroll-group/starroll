...
def test_create_job(db_session: Session):
    job = IdentifyStarsJob.create(
        db_session, user_id=1, image_key="test_image.jpg")

    assert job.id is not None
    assert job.user_id == 1
    assert job.image_key == "test_image.jpg"
    assert job.status == STAR_IDENTIFY_JOB_STATUS_PENDING
...
def test_get_by_id(db_session: Session):
    created = IdentifyStarsJob.create(
        db_session, user_id=1, image_key="test.jpg")
...
def test_soft_delete(db_session: Session):
    job = IdentifyStarsJob.create(
        db_session, user_id=1, image_key="delete_me.jpg")
    ...
    raw_fetched = db_session.query(
        IdentifyStarsJob).filter_by(id=job.id).first()
