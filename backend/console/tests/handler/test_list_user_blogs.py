...
def test_list_user_blogs_success(db_session: Session, monkeypatch: pytest.MonkeyPatch):
    """正常情况：返回该用户发的所有博客"""
    Blog.create(db_session, user_id=1, hip=100, title="My Blog 1", content="c1", image_urls=["http://img1.jpg"])
    Blog.create(db_session, user_id=1, hip=200, title="My Blog 2", content="c2", image_urls=[])
    Blog.create(db_session, user_id=2, hip=100, title="Other User Blog", content="c3", image_urls=[])
...
def test_list_user_blogs_excludes_deleted(db_session: Session, monkeypatch: pytest.MonkeyPatch):
    """软删除的博客不出现"""
    blog = Blog.create(db_session, user_id=5, hip=100, title="To Delete", content="c", image_urls=[])
    Blog.create(db_session, user_id=5, hip=100, title="Active Blog", content="c", image_urls=[])
